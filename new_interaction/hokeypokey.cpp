		...
		do{
			...
			if(status > 0)
			{
				...
				if(tokens.size() == 1) //then the message is either: repeated, wrongconfirmation, rightconfirmation, newlimb, getStarted, or actionPerformed
				{
					...
					else if(tokens[0] == "bodyDetected")
					{
						LinuxActionScript::PlayMP3("welcome.mp3");
						robotwait(16.0);
						messageToKinect = "Robot played welcome.mp3 to signify that it has detected the presence of the participant.";
					}
				...
				else //then it is a sequence of movements
				{
					...
					robotwait(10.0);
					messageToKinect = "Robot performed following sequence: \'";
					...
					//chop off the extra whitespace character at the end and add a period
					messageToKinect = messageToKinect.substr(0,messageToKinect.length()-1) + "\'.";
